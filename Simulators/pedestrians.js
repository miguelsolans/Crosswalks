...
class pedestrian{
    ...
    constructor(points, breakVelocity, standardVelocity){
        ...
        this.socketUrl = 'ws://localhost:4040/manager/pedestrian';
    }
    ...
    run(){
        ...
        socketConnection.onopen = async function(e) {
            ...
            // socketConnection.send(JSON.stringify(pedGPS));

            // Start and End, Speed
            //Primeiro cenário: Passa pedro e passa pedestres sem parar ou pausas
            ...
            //terceiro cenário: Parar na passadeira e depois arranped
            while (!ped.mr.completed()) {
                await timeout(ped.mr, 100);
            }
        ...
        let timeout = (mr, ms) => {
            return new Promise(resolve => setTimeout(function () {
                ...
                if(coords){
                    let point = new Point(userId, coords.latitude, coords.longitude, false)
        ...
        socketConnection.onmessage = function(event) {
            ...
            if(notification && notification.code){
                if(notification.code === "green"){
                    ped.mr.setVelocity(ped.standardVelocity)
                }
                else if(notification.code === "orange"){
                    ped.mr.setVelocity(ped.breakVelocity)
                }
            } else {
                console.log("unkown message " +  JSON.stringify(notification))
            }
            console.log(`ped Data Recevied: ${event.data}`);
        };
